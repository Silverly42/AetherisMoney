...
create table if not exists public.profiles (
  ...
  name text unique not null check (name in ('Lucca','Conor','Rhys','Aleesha','Tiernan')),
...
alter table public.profiles add constraint profiles_name_check
  check (name in ('Lucca','Conor','Rhys','Aleesha','Tiernan'));
...
begin
  ...
  -- Never block Supabase Authentication from creating a user. Only the five
  -- approved game accounts receive a money profile.
  if player_name is null or player_name not in ('Lucca','Conor','Rhys','Aleesha','Tiernan') then
    return new;
  end if;
...
from auth.users where lower(split_part(email,'@',1)) in ('lucca','conor','rhys','aleesha','tiernan')
