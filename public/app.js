...
async function load(){
 ...
 const allActivity=(activity.data||[]).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)||Number(b.id)-Number(a.id));
 state={users:profiles.data,requests:requests.data,transactions:transactions.data,shops:shops.data||[],products:products.data||[],preorders:preorders.data||[],notifications:notifications.data||[],activity:allActivity,trades:trades.data||[],marketReady,tradeReady:!trades.error,me:profiles.data.find(p=>p.id===user.id)}; if(!state.me)throw Error('Player profile is not ready.'); render();
}
